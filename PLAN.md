...
17. 会话模块 2026-09-05 整体重写（refactor/conversation 分支）：**存储层永不压缩**——旧实现 append 时破坏性压缩丢原文，导致归档只剩最近 12 条、title 漂移成"（无用户消息）"；压缩改为 getConversationContext 现场计算的纯函数视图。所有写操作经模块内串行队列（归档 upsert 与删除交错会互相覆盖）；title 首条用户消息时定死终身不重算；storage.local 配额写爆会拖垮 append → 归档写入带降级重试；sidepanel 事件广播改多 port（第二个面板连接曾致第一个永久失联）。会话测试组在调试面板（conversation-test.js），改本模块前先跑
18. **任务执行中开新对话不得杀 run**（2026-09-06）：旧实现 NEW_CONVERSATION 先 requestStop，长任务被一键误杀；但直接去掉 requestStop 又会让 run 的收尾输出/上下文落进新对话（appendMessage 面向活跃对话）。正解=**run 启动时绑定 convId**：appendMessageTo(convId)/getConversationContext(convId) 定向到所属对话，活跃则写 session、非活跃（用户已开新对话）则写归档对应条目——新对话随时可开，旧任务后台跑完，结果在历史面板原对话里。B8 竞态随之消解
